...
for (i = 1; i <= length(ARRAY_LOADAVG_VALUES); i++) {
    if (ONE_LOADAVG >= ARRAY_LOADAVG_VALUES[i]) { 
        ONE=AWK_ESCAPE_SEQUENCE""FG_ESCAPE_SEQUENCE""ARRAY_FG_COLORS[i]"m"AWK_ESCAPE_SEQUENCE""BG_ESCAPE_SEQUENCE""ARRAY_BG_COLORS[i]"m"ARRAY_LOADAVG[1]"\033[0m";
    }
}
# Five Minute:
for (i = 1; i <= length(ARRAY_LOADAVG_VALUES); i++) { 
    if (FIVE_LOADAVG >= ARRAY_LOADAVG_VALUES[i]) { 
        FIVE=AWK_ESCAPE_SEQUENCE""FG_ESCAPE_SEQUENCE""ARRAY_FG_COLORS[i]"m"AWK_ESCAPE_SEQUENCE""BG_ESCAPE_SEQUENCE""ARRAY_BG_COLORS[i]"m"ARRAY_LOADAVG[2]"\033[0m";
    }
}
# Fifteen Minute:
for (i = 1; i <= length(ARRAY_LOADAVG_VALUES); i++) { 
    if (FIFTEEN_LOADAVG >= ARRAY_LOADAVG_VALUES[i]) {
        FIFTEEN=AWK_ESCAPE_SEQUENCE""FG_ESCAPE_SEQUENCE""ARRAY_FG_COLORS[i]"m"AWK_ESCAPE_SEQUENCE""BG_ESCAPE_SEQUENCE""ARRAY_BG_COLORS[i]"m"ARRAY_LOADAVG[3]"\033[0m";
    }
}
