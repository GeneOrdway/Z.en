...
for (i = 1; i <= length(ARRAY_SCALE); i++) {
    if (ONE_LOADAVG >= ARRAY_SCALE[i] && AWK_TMUX_ACTIVE == 0) { 
        ONE="#[fg=colour"ARRAY_FG_COLORS[i]"]#[bg=colour"ARRAY_BG_COLORS[i]"]"ARRAY_LOADAVG[1]"#[default]";
    }
    else if (ONE_LOADAVG >= ARRAY_SCALE[i] && AWK_TMUX_ACTIVE = 1) {
...
# Five Minute:
for (i = 1; i <= length(ARRAY_SCALE); i++) { 
    if (FIVE_LOADAVG >= ARRAY_SCALE[i] && AWK_TMUX_ACTIVE == 0) { 
        FIVE="#[fg=colour"ARRAY_FG_COLORS[i]"]#[bg=colour"ARRAY_BG_COLORS[i]"]"ARRAY_LOADAVG[2]"#[default]";
    }
    else if (FIVE_LOADAVG >= ARRAY_SCALE[i] && AWK_TMUX_ACTIVE = 1) {
...
# Fifteen Minute:
for (i = 1; i <= length(ARRAY_SCALE); i++) { 
    if (FIFTEEN_LOADAVG >= ARRAY_SCALE[i] && AWK_TMUX_ACTIVE == 0) {
        FIFTEEN="#[fg=colour"ARRAY_FG_COLORS[i]"]#[bg=colour"ARRAY_BG_COLORS[i]"]"ARRAY_LOADAVG[3]"#[default]";
    }
    else if (FIFTEEN_LOADAVG >= ARRAY_SCALE[i] && AWK_TMUX_ACTIVE = 1) {
